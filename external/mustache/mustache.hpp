...
class BasicMustache {
public:
    static const bool kNoEscape = true;
...
public:
    ...
    public:
        ...
        const Data* get(const StringType& name, const FiltersType& filters) {
            // process {{.}} name
            ...
            for (const auto& item : items_) {
                ...
                for (const auto& n : names) {
                    auto filter_names = split(n, '|');
                    auto var_name = filter_names[0];
                    filter_names.erase(filter_names.begin());
...
private:
    ...
    WalkControl renderComponent(const RenderHandler& handler, Context& ctx, Component& comp) {
        ...
        switch (tag.type) {
            ...
            case Tag::Type::UnescapedVariable:
                if ((var = ctx.get(tag.name, filters_)) != nullptr) {
            ...
            case Tag::Type::SectionBeginInverted:
                if ((var = ctx.get(tag.name, filters_)) == nullptr || var->isFalse() || var->isEmptyList()) {
                    renderSection(handler, ctx, comp, var);
                }
                return WalkControl::Skip;
    ...
    bool renderLambda(const RenderHandler& handler, const Data* var, Context& ctx, bool escaped, const StringType& text, bool parseWithSameContext) {
        ...
        if (!tmpl.isValid()) {
            ...
            if (!tmpl.isValid()) {
                ...
                handler(!kNoEscape && escaped ? escape(str) : str);
            }
        }
        return tmpl.isValid();
    }
    ...
    bool renderVariable(const RenderHandler& handler, const Data* var, Context& ctx, bool escaped) {
        if (var->isString()) {
            ...
            handler(!kNoEscape &&escaped ? escape(varstr) : varstr);
