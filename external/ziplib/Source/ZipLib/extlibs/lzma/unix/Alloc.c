...
static void *VirtualAlloc_(size_t size, int memLargePages)
{
  #ifdef _7ZIP_LARGE_PAGES
  if (memLargePages)
  {
...
static int VirtualFree_(void *address)
{
  #ifdef _7ZIP_LARGE_PAGES
  #ifdef __linux__
  int i;
...
void *BigAlloc(size_t size)
{
  ...
  {
    void *res = VirtualAlloc_( (size + g_LargePageSize - 1) & (~(g_LargePageSize - 1)), 1);
    if (res != 0)
      return res;
  }
  #endif
