...
void test() {
    ...
    if (0) {
        ...
        {
            auto h = handlers::JsonHandler(yaml_config);
            cnv.run(h);
        }
        {
            auto h = handlers::DjangoFixtureHandler(yaml_config);
            cnv.run(h);
        }
        {
            auto h = handlers::CSVHandler(yaml_config);
            cnv.run(h);
        }
        {
            auto h = handlers::LuaHandler(yaml_config);
            cnv.run(h);
        }
    }
}
